...
private[spark] class BarebonesBroadcast[T: ClassTag](obj: T, id: Long)
extends Broadcast[T](id) with Logging with Serializable {
  ...
  private def writeBroadcastBlocks(value: T): Int = {
    ...
    blocks.zipWithIndex.foreach { case (block, i) =>
      val pieceId = BroadcastBlockId(id, "block" + i)
      val bytes = new ChunkedByteBuffer(block.duplicate()) // remove duplicate
      if (!bm.putBytes(pieceId, bytes, MEMORY_AND_DISK_SER, tellMaster = true)) {
  ...
  private def readBroadcastBlocks(): T = Utils.tryOrIOException {
    ...
    // use torrent as a synchronized source, because it has an object
      val startTimeMs = System.currentTimeMillis()
      val bm = SparkEnv.get.blockManager
...
private[spark] class BarebonesBroadcastFactory extends BroadcastFactory {
